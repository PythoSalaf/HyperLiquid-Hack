  ...
  return (
    ...
    <div className="w-full fixed bg-[#0f1724] border-b  border-b-[#202a3a] flex items-center text-white py-3">
      <div className="w-[95%] mx-auto md:w-[92%] flex items-center justify-between">
        ...
        <div className="flex">
          <button style = {{display: authenticated ? 'none' : 'flex'}} onClick={login} className="hidden active:bg-blue-800 active:scale-95 transition-transform duration-300 ease-in-out 
          md:block bg-[#5b8eff] text-white px-3 ml-4 py-1.5 rounded-lg">
            Login / Sign Up
          </button>
          <button  style = {{display: authenticated ? 'flex' : 'none'}} onClick={logout} className="hidden active:bg-blue-800 active:scale-95 transition-transform duration-300 ease-in-out 
          md:block bg-[#5b8eff] text-white px-3 ml-4 py-1.5 rounded-lg">
            Logout
          </button>
